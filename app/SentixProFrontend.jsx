    ...
    const loadPaperData = async () => {
      ...
      try {
        const [configRes, posRes, histRes, perfRes] = await Promise.allSettled([
          ...
          authFetch(`${API_URL}/api/positions/${USER_ID}`),
      ...
      <div>
        ...
        <div style={{ ...card, padding: "16px 20px" }}>
          <div style={{ ...sTitle, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span>POSICIONES ABIERTAS ({paperPositions.length})</span>
            <span style={{
              fontSize: 9, fontWeight: 700, padding: "2px 8px", borderRadius: 4,
              background: executionMode === 'bybit' ? "rgba(255,168,0,0.15)" : "rgba(96,165,250,0.15)",
              color: executionMode === 'bybit' ? yellow : "#60a5fa",
              fontFamily: "monospace"
            }}>
              {executionMode === 'bybit' ? (bybitTestnet ? 'BYBIT TESTNET' : 'BYBIT LIVE') : 'PAPER'}
            </span>
          </div>
          {paperPositions.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {paperPositions.map(pos => {
                const isLong = pos.direction === 'LONG' || pos.side === 'BUY';
                const pnlColor = (pos.unrealizedPnl || pos.unrealized_pnl || 0) >= 0 ? green : red;
                const isUntracked = pos.source === 'bybit-untracked';
                const sourceBadge = pos.source === 'paper' ? { label: 'PAPER', color: '#60a5fa', bg: 'rgba(96,165,250,0.15)' }
                  : pos.source === 'bybit-tracked' ? { label: 'BYBIT', color: yellow, bg: 'rgba(255,168,0,0.15)' }
                  : pos.source === 'bybit-untracked' ? { label: 'HOLDING', color: muted, bg: 'rgba(128,128,128,0.15)' }
                  : null;
                return (
                  <div key={pos.id || pos.coin || pos.asset} style={{
                    background: bg3, borderRadius: 8, padding: "12px 16px",
                    borderLeft: `3px solid ${isUntracked ? muted : (isLong ? green : red)}`,
                    ...
                    <div style={{ minWidth: 120 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ color: isUntracked ? muted : (isLong ? green : red) }}>{isUntracked ? '○' : (isLong ? '▲' : '▼')}</span>
                        {pos.asset}
                        {sourceBadge && (
                          <span style={{ fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 3, background: sourceBadge.bg, color: sourceBadge.color, fontFamily: "monospace" }}>
                            {sourceBadge.label}
                          </span>
                        )}
                      </div>
                      <div style={{ fontSize: 10, color: muted, fontFamily: "monospace" }}>
                        {isUntracked ? (pos.sourceLabel || 'Not managed by SENTIX') : `${pos.direction || pos.side} · ${pos.entry_signal_strength || 'N/A'} · ${formatDuration(pos.entry_at || pos.order_created_at)}`}
                      </div>
                    </div>
                    ...
                    <div style={{ textAlign: "center", minWidth: 100 }}>
                      ...
                      <div style={{ fontSize: 12, fontFamily: "monospace", fontWeight: 600 }}>
                        {Number(pos.entry_price) > 0 ? `$${Number(pos.entry_price).toLocaleString()}` : '—'} → <span style={{ color: pnlColor }}>${(pos.currentPrice || pos.current_price || 0).toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div style={{ textAlign: "right", minWidth: 90 }}>
                      <div style={{ fontSize: 14, fontWeight: 800, fontFamily: "monospace", color: pnlColor }}>
                        {(pos.unrealizedPnl || pos.unrealized_pnl || 0) >= 0 ? '+' : ''}${(pos.unrealizedPnl || pos.unrealized_pnl || 0).toFixed(2)}
                      </div>
